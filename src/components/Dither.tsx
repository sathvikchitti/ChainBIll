...
// ─── Retro dither post-process effect ───────────────────────────────────────
...
class RetroEffectImpl extends Effect {
  ...
  set colorNum(v: number) { (this.uniforms as Map<string, THREE.Uniform<number>>).get('colorNum')!.value = v }
  get colorNum(): number { return (this.uniforms as Map<string, THREE.Uniform<number>>).get('colorNum')!.value }
  set pixelSize(v: number) { (this.uniforms as Map<string, THREE.Uniform<number>>).get('pixelSize')!.value = v }
  get pixelSize(): number { return (this.uniforms as Map<string, THREE.Uniform<number>>).get('pixelSize')!.value }
}

// Cast away the broken wrapEffect generic so JSX works
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const WrappedRetro = wrapEffect(RetroEffectImpl) as any

interface RetroEffectProps { colorNum?: number; pixelSize?: number }
